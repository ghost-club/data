...
module Types =
    ...
    type Body =
        abstract body: Node.Stream.Readable<string>
        abstract bodyUsed: bool with get, set
        abstract buffer: unit -> JS.Promise<Node.Buffer.Buffer>
    ...
    type [<Erase>] GlobalFetch =
        [<ImportDefault("node-fetch")>]
        static member fetch (req: RequestInfo, ?init: RequestInit) = jsNative :JS.Promise<Response>
